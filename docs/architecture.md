...
O botão de conexão no topo está ativo quando `VITE_GOOGLE_CLIENT_ID` está definido. O cliente OAuth Web e a API do Drive estão configurados no projeto Google Cloud `biorotina`. O token de acesso fica no armazenamento local do navegador e é renovado sem interação quando o Google permite; o modelo atual não usa refresh token no servidor. Se a renovação falhar com o app aberto, os registros locais desta conta permanecem na aba e só a sincronização é pausada. A reconexão por toque reaproveita a permissão do Drive já concedida. Após recarregar com o token expirado, os registros da conta ficam ocultos até a reconexão, sem serem apagados.
