...
| B04 | **Antes de esgotar a cota**, escolher a imagem pública `public/icon-512.png` e analisar. Comparar contador antes/depois. | IA não inventa comida; aparece mensagem compreensível para foto sem refeição e o cadastro manual continua possível. Uma resposta não aproveitada não deve consumir a cota. **Risco conhecido a verificar:** o backend atual rejeita `foods` vazio e pode exibir erro genérico 503. |
| B05 | Conferir o contador. Repetir análises válidas, uma de cada vez, até completar no máximo as cinco análises gratuitas **da conta**. | Uma análise bem-sucedida consome uma unidade; saldo mostrado acompanha o backend. Não fazer chamadas extras só para forçar o limite. |
| B06 | Quando a conta chegar a 5/5, tocar para usar foto novamente. | Diálogo oferece plano ou chave Gemini e deixa explícito que a refeição pode ser registrada manualmente. Não abre câmera antes do aviso. |
