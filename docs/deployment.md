...
O workflow `.github/workflows/ci.yml` valida o frontend e o Rust em pull requests para `master`. Em push/merge em `master`, o deploy só começa depois de todas as verificações passarem. O acesso AWS usa OpenID Connect com a role `biorotina-github-deploy`; não existe AWS access key no GitHub nem no código. A política de confiança limita o acesso ao repositório `ewflaviano/biorotina` na branch `master`.
